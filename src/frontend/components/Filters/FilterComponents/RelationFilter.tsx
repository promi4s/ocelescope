...
const RelationFilter: React.FC<{
  ...
  control: FilterPropsType<"e2o_count" | "o2o_count">["control"];
  index: number;
  remove: () => void;
}> = memo(({ control, index, soureRelations, targetRelations, remove }) => {
  ...
  return (
    <Grid align="center" gutter={"md"}>
      ...
      <Grid.Col span={4} display={"flex"} style={{ justifyContent: "center" }}>
        <Controller
          control={control}
          name={`value.${index}.qualifier`}
          render={({ field }) => (
            <Select
              data={qualifierNames}
              label={"Qualifier"}
...
export const E2OCountFilter: React.FC<FilterPropsType<"e2o_count">> = ({
  ocelParams,
  control,
}) => {
...
export const O2OCountFilter: React.FC<FilterPropsType<"o2o_count">> = ({
  ocelParams,
  control,
}) => {
