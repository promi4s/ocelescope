  ...
  >
    ...
    {initialTokens && !innerSymbol && (
      initialTokens === 1 ? (
        ...
        <span style={{ color: "#111", fontSize: 11, fontWeight: 700, lineHeight: 1 }}>
          {initialTokens}
        </span>
      )
    )}
...
const GraphFlowNode = memo(({ data }: NodeProps<GraphFlowNodeType>) => {
  ...
  const isExternalLabel = Boolean(label && labelPos !== "center" && labelPos != null);
  ...
  return (
    ...
    >
      ...
      {isExternalLabel && labelPos === "top" && (
        <NodeLabel absolute top={0}>{label as string}</NodeLabel>
      )}
      {shape === "circle" ? (
        <CircleShape
      ...
      {isExternalLabel && labelPos === "bottom" && bottomLabelTop != null && (
        <NodeLabel absolute top={bottomLabelTop}>{label as string}</NodeLabel>
      )}
